...
class DiscordUserMap {
	...
	constructor(filename) {
		...
		try {
			...
			Application.logger.warn(`Invalid JSON in ${this._filename}:\n\n${data}\n\nStarting with empty usermap`);
			data = {};
		}
	...
	_saveMap() {
		...
		this._finishedWriting = this._finishedWriting
		  .then(() => writeFileAsync(this._filename, JSON.stringify(this._idToName, null, "\t")))
		  .catch((err) => console.error("Writing discord user map failed!", err));
	}
