 ...
 */
async function setup(dcBot, tgBot, dcUsers, messageMap) {
		...
		if (Application.bridgeMap.has("discord", message.channel.id)) {
			...
			message.attachments.forEach(async ({url}) => {
				try {
					...
					Application.logger.error(`[${bridge.name}] Telegram did not accept an attachment:`, err);
				}
			});
			...
			message.embeds.forEach((embed) => {
				...
				try {
					...
					Application.logger.error(`[${bridge.name}] Telegram did not accept an embed:`, err);
				}
			});
	...
	dcBot.on("messageUpdate", async (oldMessage, newMessage) => {
		// XXX This actually doesn't appear to trigger on messages from before the bot started. Not that it matters,
		// as the map to Telegram IDs are not stored between runs
		...
		try {
			...
			const tgMessageId = messageMap.getCorresponding(MessageMap.DISCORD_TO_TELEGRAM, newMessage.id);
			...
			Application.logger.error(`[${bridge.name}] Could not edit Telegram message:`, err);
		}
	});
	...
	dcBot.on("disconnected", (evt) => {
		...
		// Tell the telegram side of each bridge that the Discord connection died
		Application.bridgeMap.maps.forEach(async (bridge) => {
			try {
				...
				Application.logger.error(`[${bridge.name}] Could not send message to Telegram:`, err);
			}
		  });
	});
	...
	if (!Application.settings.discord.skipOldMessages) {
		...
		dcBot.ready = dcBot.ready.then(async () => {

			// Find the latest message IDs for all bridges
 			const objs = Application.bridgeMap.maps.map((bridge) => ({
				bridge,
			...
			await Promise.all(objs.map(async ({bridge, messageId}) => {
				try {
					const messages = await dcBot.channels.get(bridge.discord.channel).fetchMessages({limit: 100, after: messageId})
					...
					// Could not relay old messages
					Application.logger.error(`[${bridge.name}] Could not relay old Discord messages:`, err);
				}
			}));
		});
	}
	...
	try {
		await dcBot.login(Application.settings.discord.auth.token);
	} catch (err) {
		Application.logger.error("Could not authenticate the Discord bot:", err);
	}
}
