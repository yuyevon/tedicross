...
function makeFileSender(tgBot, dcBot, dcUsers) {
	...
	return async function({discordChannel, message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		fileStream.on("end", async () => {
			try {
				await dcBot.channels.get(discordChannel).send(
					textToSend,
					{
						file: {
				...
				Application.logger.error("Discord did not accept a file:", err);
			}
		});
	};
}
...
function setup(tgBot, dcBot, dcUsers, messageMap) {
	...
	tgBot.on("text", wrapFunction(async (message, bridge) => {
		...
		// Get the message on a discord friendly format
		let messageObj = messageConverter({ message, dcUsers, tgBot });
	...
	tgBot.on("photo", wrapFunction(async (message, bridge) => {
		try {
			...
			Application.logger.error("Could not send photo", err);
		}
	}));
	...
	tgBot.on("sticker", wrapFunction(async (message, bridge) => {
		try {
			...
			Application.logger.error("Could not send sticker", err);
		}
	}));
	...
	tgBot.on("document", wrapFunction(async (message, bridge) => {
		try {
			...
			Application.logger.error("Could not send document", err);
		}
	}));
	...
	tgBot.on("voice", wrapFunction(async (message, bridge) => {
		try {
			...
			Application.logger.error("Could not send voice", err);
		}
	}));
	...
	tgBot.on("audio", wrapFunction(async (message, bridge) => {
		try {
			...
			Application.logger.error("Could not send audio", err);
		}
	}));
	...
	tgBot.on("video", wrapFunction(async (message, bridge) => {
		try {
			...
			Application.logger.error("Could not send video", err);
		}
	}));
